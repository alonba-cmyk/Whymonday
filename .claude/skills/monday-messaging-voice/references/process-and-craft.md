...
- **Alon's naming taste (observed).** He leans to the playful **magic-reveal / incantation** register and to names carrying a **clever double or bilingual meaning**. In the vibe-coding naming round his favorite was **Abra** — abracadabra plus the Aramaic *avra k'davra* ("I create as I speak"), which doubles as the product thesis: you speak, it's built. Offer that register first for him: warm, playful, a wink of hidden meaning that pays off the thesis. Practical catch he ran into: the whole magic-word family (Abra, Cadabra, Voila, Spell, Ta-da/Tada, Presto, Avra, Wand, Imagine, Knack) is already trademarked in or beside software, so pair any magic-family pick with an early domain + trademark reality check, and keep a coined cousin ready (e.g. Abrio) as the ownable fallback.
...
- Every rendered page ships **theme-aware (full light and dark)** with a **light/dark toggle in the header**, and **defaults to light** on load (the toggle switches to dark). Force the light default at the top of the page (`document.documentElement.setAttribute('data-theme','light')`), since most viewers consume more comfortably in light and shared pages open outside claude.ai without the viewer's own theme control.
